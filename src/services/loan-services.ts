...
async function registerLoan(itemId: any, usuario: Usuario) {
    ...
    if(item.rows[0].isbn) {
        ...
        const bookExists = await materialRepository.findBookByISBN(item.rows[0].isbn)
        ...
        const materialExists = await materialRepository.findMaterialById(item.rows[0].id_material)
