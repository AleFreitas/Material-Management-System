...
import {
  Livro,
  PayloadRegisterAuthor,
  PayloadRegistroMaterial,
...
async function findBookAuthorRelation(
  id: any,
  isbn: any
): Promise<QueryResult> {
  return pool.query(
...
async function insertMaterial(
  material: PayloadRegistroMaterial
): Promise<QueryResult> {
  return pool.query(
    `
...
async function linkBookToCategory(
  isbn: string,
  category_id: number
): Promise<QueryResult> {
  return pool.query(
...
async function findBookCategoryRelation(
  isbn: string,
  category_id: number
): Promise<QueryResult> {
  return pool.query(
...
async function findAllBooksWithRelations(): Promise<QueryResult> {
  return pool.query(`
      SELECT 
        livro.*,
        STRING_AGG(DISTINCT autor.nome, ', ') AS autores,
        STRING_AGG(DISTINCT categoria_livro.nome, ', ') AS categorias
      FROM livro
